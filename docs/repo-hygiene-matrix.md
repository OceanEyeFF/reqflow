...
| Local safety backups | `.local-backup/prisma-dev-db-before-wt022/dev.db` | untracked local backup directory | keep local-only until the owner decides the backed-up DB is no longer needed; do not commit and do not delete automatically | `WT-20260524-024` or later targeted cleanup | Medium risk. It may contain local data preserved before DB untracking. Treat deletion as a user-owned cleanup decision, not a repo hygiene shortcut. |
